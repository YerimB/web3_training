import pytest
from brownie import exceptions, TimedLottery
...
def test_owner_can_start_timed_lottery():
    only_local()

    # Init
    owner = get_account(0)
    contract = deploy_lottery(account=owner, lottery_type=TimedLottery)

    # Core / Assert
    # --- Check lottery is closed
    assert contract.m_LotteryState() == LotteryState.CLOSED.value
    # Open lottery
    tx = TransactionReceipt(contract.startLottery(60, {"from": owner}).txid)
